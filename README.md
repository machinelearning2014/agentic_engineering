...
These rules are also encoded in `AGENTS.md`, which most agents load automatically —
so you don't have to re-explain the boundaries on every task.
...
- **Per task**: run `./ae new <task>` (or manually `cp spec-template.md tasks/<task>/spec.md`)
  and fill it in.
...
| `AGENTS.md` | — | Standing instructions the AI agent reads before working |
...
Scaffold a task with `./ae new <task>` (or, manually,
`cp spec-template.md tasks/<task-name>.md`), then fill in EVERY field before
delegating. The filled file is what you hand to the agent. A blank field is a gap
the agent will fill with something plausible-but-wrong.
...
`./ae` turns the loop into one command per stage (plain bash, no dependencies):

| Command | What it does |
| --- | --- |
| `./ae new <task>` | Scaffold `tasks/<task>/spec.md` from the template |
| `./ae verify [<task>]` | Lint: no blank fields, criteria executable, exactly one publication-intent checkbox |
| `./ae blocker <task>` | Scaffold `tasks/<task>/blocker.md` |
| `./ae accept <task> --evidence "…"` | Record an ACCEPT decision (refuses unless verify passes) |
| `./ae reject <task> --reason "…"` | Record a REJECT decision |
| `./ae status` | Show each task's verify + decision state |

`verify` is CI-friendly: it exits non-zero on a failing spec.
